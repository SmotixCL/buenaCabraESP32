...
void BuzzerManager::initializeAlertConfigs() {
    ...
    alertConfigs[static_cast<uint8_t>(AlertLevel::WARNING)].interval = 3000;    // Cada 3s
    alertConfigs[static_cast<uint8_t>(AlertLevel::DANGER)].interval = 1000;     // Cada 1s
    alertConfigs[static_cast<uint8_t>(AlertLevel::EMERGENCY)].interval = 500;   // Cada 0.5s
}
