...
enum class Result : uint8_t {
    SUCCESS = 0, ERROR_INIT, ERROR_TIMEOUT, ERROR_INVALID_PARAM, ERROR_NO_MEMORY, 
    ERROR_HARDWARE, ERROR_COMMUNICATION, ERROR_GPS_NO_FIX, ERROR_BATTERY_LOW
};

// --- Enum de Nivel de Alerta ---
enum class AlertLevel : uint8_t {
    SAFE = 0, CAUTION, WARNING, DANGER, EMERGENCY
};

// --- Tipos de Geocerca ---
enum class GeofenceType : uint8_t { CIRCLE = 0, POLYGON };

// --- Estructuras de Datos ---
struct Position {
    double latitude = 0.0; double longitude = 0.0; float altitude = 0.0;
    float accuracy = 99.0; uint8_t satellites = 0; uint32_t timestamp = 0; bool valid = false;
};
struct BatteryStatus {
    float voltage = 0.0; uint8_t percentage = 0; bool charging = false;
    bool low = false; bool critical = false; uint32_t lastReading = 0;
};
struct SystemStatus {
    bool buzzerInitialized, displayInitialized, gpsInitialized, radioInitialized;
    uint32_t uptime; uint32_t freeHeap; uint8_t currentState;
};
struct SystemStats {
...
struct Geofence {
    GeofenceType type; bool active; bool isConfigured; char name[32]; char groupId[16];
    double centerLat; double centerLng; float radius;
    static const uint8_t MAX_POLYGON_POINTS = 10;
    GeoPoint points[MAX_POLYGON_POINTS]; uint8_t pointCount;
...
struct AlertConfig {
    bool enabled; uint16_t frequency; uint16_t duration; uint8_t volume;
    uint16_t interval; uint8_t repetitions;
